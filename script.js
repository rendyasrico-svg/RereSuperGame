...
imagesToLoad.forEach(img => {
    ...
    img.onerror = () => {
        // Tetap lanjut meski ada gambar gagal load
        imagesReady++;
        if (imagesReady >= imagesToLoad.length) {
            allImagesLoaded = true;
        }
...
// Fungsi aman untuk play audio (tidak crash kalau file tidak ada)
...
// ====================
// DRAW BACKGROUND (dengan fallback)
// ====================

function drawBackground() {
    // Cek apakah gambar background berhasil dimuat
    if (bg.complete && bg.naturalWidth > 0) {
        ctx.drawImage(bg, 0, 0, canvas.width, canvas.height);
    } else {
        // Fallback: gradient langit biru
        ...
        // Tambah sedikit awan putih sebagai hiasan fallback
...
function draw() {
    ...
    // Background (dengan fallback agar tidak hitam)
    drawBackground();
